...
%% Author: Oscar Ortiz
 ...
 % Inputs
 %    - The two data matrices you want to compare (dimensions: number of subjects X
 %      number of frequencies X number of time bins).
 %    - vector delimiting the time stamps (in ms)
 %    - vector delimiting the central frequencies extracted. 
 %    - number of permutations you want to run the data for. 



pre_stim_data = cond1_Ci; % Placebo cheps
post_stim_data = cond3_Ci; % Placebo Leps
n_subs = 17;

%% getting parameters for permutations
times = cond_1.times;
frequencies = cond_1.frex;
frequencies(2) = 33.0;
frequencies(4) = 37.0;
time_s = dsearchn(times',-150); % start time for tf window
time_e = dsearchn(times',800); % end time for tf window
tftimes  = times(time_s:time_e);
...
temp = squeeze(pre_stim_data(1,:,time_s:time_e));
...
for i = 1:n_subs
    % for pre
    curr = squeeze(pre_stim_data(i,:,time_s:time_e));
    curr_vect = curr(:);
    pre_mat(i,:) = curr_vect';
  
    % for post
    curr = squeeze(post_stim_data(i,:,time_s:time_e));
    curr_vect = curr(:);
...
permuted_maxvals = zeros(n_permutes,2,length(frequencies));
permuted_vals    = zeros(n_permutes,length(frequencies),nTimepoints);
...
my_hxlim = 950;
subplot(2,2,1)
contourf(tftimes,frequencies,zmap,40,'linecolor','none')
axis square
...
hold on
contour(tftimes,frequencies,zmapthresh,1,'linecolor','k')
c = colorbar;
c.Label.String = 'Z score';
...
subplot(2,2,3)
contourf(tftimes,frequencies,zmapthresh,40,'linecolor','none')
axis square
set(gca,'clim',[-5 5],'xlim',[my_lxlim my_hxlim])
